...
must_haves:
  truths:
    ...
    - "No copied BOM functions remain in test/run-tests.js (the ~430 lines of duplicated code are deleted)"
  ...
  key_links:
    ...
    - from: "test/run-tests.js"
      to: "js/core/tree.js"
      via: "import { BOMNode, buildTree } from '../js/core/tree.js'"
...
**Step 3: Keep ALL test infrastructure functions intact**
...
**Step 4: Keep ALL test case functions intact**

Keep these functions exactly as they are:
- `test1_FlatBOM_XML()`
...
**Step 5: Verify the rewritten file structure is clean**
...
Also verify no copied functions remain:
```bash
grep -n "EXTRACTED FUNCTIONS" test/run-tests.js
```
Expected: No matches (the comment block is deleted).

Verify test execution time is under 10 seconds:
```bash
time node test/run-tests.js
```
  </verify>
  <done>test/run-tests.js imports all BOM functions from js/core/ modules. Zero copied production functions remain. All 4 tests pass with identical results. Execution under 10 seconds.</done>
</task>
...
- Rewrote test/run-tests.js to import from modules (~430 lines of copied code deleted)
...
2. All 4 validation tests pass: `cd test && node run-tests.js` shows 4/4 passed
3. Test execution completes in under 10 seconds
4. `.planning/phases/01-test-infrastructure/SMOKE-TEST.md` exists with checklist for all 3 tabs
5. `index.html` is UNCHANGED (browser still works from single file)
6. Module dependency chain is clean: utils <- tree <- flatten, parser, compare (no circles)
</verification>
