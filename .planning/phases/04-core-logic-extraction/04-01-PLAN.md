...
Add this sortChildren function inside buildTree, after `const root = nodes.get('1')` and the root null check, BEFORE the root info capture lines (`_rootPartNumber = root.partNumber` etc.):
...
CRITICAL ORDERING: `sortChildren(root)` MUST appear BEFORE the `_rootPartNumber = root.partNumber` assignment line. This ensures sorting happens before root info capture, matching the inline HTML behavior.
...
Note: `resetRootInfo` is exported from tree.js (NOT compare.js), because tree.js owns the root info private variables.
...
Verify sortChildren(root) appears BEFORE _rootPartNumber assignment in tree.js:
```bash
grep -n "sortChildren(root)\|_rootPartNumber" js/core/tree.js
```
Expected: sortChildren(root) line number is LOWER than _rootPartNumber assignment line number
  </verify>
  <done>
tree.js exports BOMNode, buildTree (with sortChildren called before root info capture), getRootPartNumber, getRootRevision, getRootDescription, and resetRootInfo. compare.js compareBOMs returns results with attributesChanged and lengthFractional fields. All 4 tests pass.
  </done>
</task>
...
7. Verify compareBOMs call site was correctly updated:
   Run: `grep -n "comparisonResults = compareBOMs" C:/Users/amcallister/Projects/bom-tool/index.html` -- returns exactly 1 match containing `compareBOMs(oldBomFlattened, newBomFlattened)`
   Run: `grep -c "compareBOMs()" C:/Users/amcallister/Projects/bom-tool/index.html` -- returns 0 (no zero-arg calls remain)
  </verify>
  <done>
index.html imports BOMNode, buildTree, getRootPartNumber, getRootRevision, getRootDescription, resetRootInfo from tree.js; flattenBOM, sortBOM from flatten.js; parseXML from parser.js; compareBOMs, extractSubtree from compare.js. Zero inline core logic definitions remain. compareBOMs call site passes params and captures return value (exactly 1 match for `comparisonResults = compareBOMs(oldBomFlattened, newBomFlattened)`, zero matches for `compareBOMs()`). All root info reads use getter functions. Root info reset uses resetRootInfo(). All 4 automated tests pass with identical outputs.
  </done>
</task>
...
6. compareBOMs call site uses params + return value pattern (exactly 1 match, zero old-style calls)
7. tree.js buildTree sorts children recursively (sortChildren(root) called BEFORE _rootPartNumber assignment)
...
- Module compare.js compareBOMs returns objects with attributesChanged and lengthFractional fields (compare.js does NOT export resetRootInfo -- that is in tree.js)
- compareBOMs call site in index.html uses `comparisonResults = compareBOMs(oldBomFlattened, newBomFlattened)` pattern
- sortChildren(root) call in tree.js appears before _rootPartNumber assignment
</success_criteria>
