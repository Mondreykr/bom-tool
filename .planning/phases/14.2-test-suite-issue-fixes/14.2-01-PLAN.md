...
<task type="auto">
  <name>Task 1: Resolve source labels in merge output and UI display</name>
  <files>js/core/merge.js, js/ui/ifp-merge.js</files>
  <action>
...
**ifp-merge.js — display resolved source labels in renderTreeNode:**
...
**ifp-merge.js — update toggle button dataset.source values:**
- In `renderTreeNode`: set `row.dataset.source = 'grafted'` when `node._source` starts with 'B(', set `row.dataset.source = 'placeholder'` when `node._source === 'New (WIP)'`, and set `row.dataset.source = 'current'` when `node._source` starts with 'X('. The hide-grafted toggle should continue to hide rows where `dataset.source === 'grafted'`.
  </action>
  <verify>
- After merge with prior artifact: source column shows "X(2)", "B(1)", "New (WIP)" — never blank, never "B(n-1)"
- Toggle buttons still correctly target rows via dataset.source values ('grafted', 'placeholder', 'current')
  </verify>
  <done>
Source labels resolve to actual revision numbers in all user-facing text. renderTreeNode displays resolved labels with appropriate styling. dataset.source values remain toggle-compatible.
  </done>
</task>

<task type="auto">
  <name>Task 2: Improve validation messages and fix 258758 XML false positives</name>
  <files>js/core/merge.js, js/core/validate.js</files>
  <action>
...
- Warning for missing assembly includes description in parentheses
  ...
  <done>
Warnings include descriptions in parentheses and resolved revision numbers. Validation errors include raw PDM state in brackets. 258758 XML false positives eliminated by skipping empty-PN nodes.
  </done>
</task>

<task type="auto">
  <name>Task 3: Fix job number conditional logic and IFP revision auto-increment</name>
