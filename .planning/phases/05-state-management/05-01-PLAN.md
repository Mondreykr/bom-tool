...
1. Check file exists: `ls js/ui/state.js`
2. Test import and key count (two separate commands):
   - `node --input-type=module -e "import { state } from './js/ui/state.js'; console.log('Keys:', Object.keys(state).length);"` — should print 22
   - `node --input-type=module -e "import { state } from './js/ui/state.js'; console.log('csvData:', state.csvData); console.log('currentFilter:', state.currentFilter);"` — should show null and 'all'
  </verify>
...
1. **Simple reads:** `csvData` → `state.csvData`
2. **Assignments:** `csvData = parseXML(xmlText);` → `state.csvData = parseXML(xmlText);`
3. **Null checks:** `if (!flattenedBOM)` → `if (!state.flattenedBOM)`
4. **Method/property access:** `flattenedBOM.map(...)` → `state.flattenedBOM.map(...)`
5. **Property access:** `flattenedBOM.length` → `state.flattenedBOM.length`
6. **Template literals:** `` `${uploadedFilename}` `` → `` `${state.uploadedFilename}` ``
7. **Ternary/fallback:** `uploadedFilename || 'N/A'` → `state.uploadedFilename || 'N/A'`
8. **Reset patterns:** `csvData = null;` → `state.csvData = null;`
9. **Chained expressions:** `flattenedBOM = sortBOM(items)` → `state.flattenedBOM = sortBOM(items)`
